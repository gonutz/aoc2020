...
func main() {
	cur := input
	for {
		next := nextStateAdjacent(cur)
		if cur == next {
			fmt.Println(strings.Count(cur, "#"), "eventually seated (adjacent)")
			break
		}
		cur = next
	}
	...
	cur = input
	for {
		next := nextStateVisible(cur)
		if cur == next {
			fmt.Println(strings.Count(cur, "#"), "eventually seated (visible)")
			break
		}
		cur = next
	}
...
func nextStateAdjacent(cur string) string {
	lines := bytes.Split([]byte(cur), []byte("\n"))
	seat := func(x, y int) int {
	...
	next := bytes.Split([]byte(cur), []byte("\n"))
	...
	return string(bytes.Join(next, []byte("\n")))
}

func nextStateVisible(cur string) string {
	lines := bytes.Split([]byte(cur), []byte("\n"))
	seat := func(x, y, dx, dy int) int {
		for {
			x += dx
			y += dy
			if x < 0 || x >= len(lines[0]) ||
...
const input = `LLLLLL.LLLLLLLLLLLL.LLLLLL.LLLLLLLLLLLL.LLLLLLLLLLLLL.LLLLLLLLLLLLLLLLLLLLLLLLL.LLLLLLLL.LLLLLLLLL
