...
func main() {
	...
	for i := range passports {
		...
		fields := strings.Split(passports[i], " ")
		have := make(map[string]bool)
		for _, f := range fields {
			name := strings.Split(f, ":")[0]
			have[name] = true
		}
		if have["byr"] && have["iyr"] && have["eyr"] && have["hgt"] &&
			have["hcl"] && have["ecl"] && have["pid"] {
			simpleValid++

			// NOTE to myself: learn regular expressions.
			validValues := true
			for _, f := range fields {
				...
				case "byr":
					...
					if !(err == nil && 1920 <= n && n <= 2002) {
						validValues = false
					}
				case "iyr":
					n, err := strconv.Atoi(value)
					if !(err == nil && 2010 <= n && n <= 2020) {
						validValues = false
					}
				case "eyr":
					n, err := strconv.Atoi(value)
					if !(err == nil && 2020 <= n && n <= 2030) {
						validValues = false
					}
				case "hgt":
					n, err := strconv.Atoi(value[:len(value)-2])
					if strings.HasSuffix(value, "cm") {
						if !(err == nil && 150 <= n && n <= 193) {
							validValues = false
						}
					} else if strings.HasSuffix(value, "in") {
						if !(err == nil && 59 <= n && n <= 76) {
							validValues = false
						}
					} else {
						validValues = false
	...
	fmt.Println(simpleValid, "have all required fields")
	fmt.Println(allValid, "are fully valid")
}
