...
func main() {
	bags := make(map[string][]colorCount)
	containment := regexp.MustCompile(`(.*) bags contain (.*).`)
	items := regexp.MustCompile(`(\d+) (.*) bags?`)
	for _, line := range strings.Split(input, "\n") {
		s := containment.FindStringSubmatch(line)
		color := s[1]
		if s[2] == "no other bags" {
			bags[color] = []colorCount{}
		} else {
			hasList := strings.Split(s[2], ", ")
			for _, s := range hasList {
				sub := items.FindStringSubmatch(s)
				n, _ := strconv.Atoi(sub[1])
				bags[color] = append(bags[color], colorCount{
					color: sub[2],
					count: n,
				})
			}
	...
	var hasGold func(color string) bool
	hasGold = func(color string) bool {
		for _, cc := range bags[color] {
			if cc.color == "shiny gold" {
				return true
			}
		}
		for _, cc := range bags[color] {
	...
	fmt.Println(sumHasGold, "bags can contain a shiny golden bag")

	var bagCount func(color string) int
	bagCount = func(color string) int {
		sum := 0
		for _, cc := range bags[color] {
			sum += cc.count * (1 + bagCount(cc.color))
		}
		return sum
	}
	fmt.Println("a shiny golden bag contains", bagCount("shiny gold"), "bags")
}
